...
export const updateProductAction = createAsyncThunk('product/update',
  async (payload, { rejectWithValue, getState, dispatch }) => {
    try {
      // const { name, description, category, sizes, brand, colors, price, totalQty,images } = payload;
      ...
      // make request
      const { data } = await axios.put(`${baseURL}/products/${id}/update`, { name, description, category,sizes, brand, colors,
        price, totalQty }, config);
      return data;
    } catch (error) {
      return rejectWithValue(error?.response?.data);
    }
