              ...
              {cartItems?.map((product) => (
                <li key={product._id} className="flex py-6 sm:py-10">
                  <div className="flex-shrink-0">
                    ...
                    <img src={`data:${product.image.contentType};base64,${product.image.data.toString("base64")}`} alt={product.name} className="h-24 w-24 rounded-md object-cover object-center sm:h-48 sm:w-48"/>
                  </div>
                  ...
                  <div className="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                    <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                      ...
                      <div className="mt-4 sm:mt-0 sm:pr-9">
                        ...
                        <select onChange={(e) => changeOrderItemQtyHandler(product?._id, e.target.value)} className="max-w-full rounded-md border border-gray-300 py-1.5 text-left text-base font-medium leading-5 text-gray-700 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
                          {/* use the qty  */}
                        ...
                        <div className="absolute top-0 right-0">
                          <button onClick={() => removeOrderItemHandler(product?._id)} className="-m-2 inline-flex p-2 text-gray-400 hover:text-gray-500">
          ...
          <section  aria-labelledby="summary-heading" className="mt-16 rounded-lg bg-gray-50 px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8">
            ...
            <dl className="mt-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <dd className="text-sm font-medium text-gray-900">IRR {sumTotalPrice}</dd>
              </div>
