          ...
          <div className="relative box-content h-80 overflow-x-auto py-2 xl:overflow-visible">
            <div className="min-w-screen-xl absolute flex space-x-8 px-4 sm:px-6 lg:px-8 xl:relative xl:grid m-2  xl:grid-cols-5 xl:gap-x-8 xl:space-x-0 xl:px-0">
              {categories?.map((category) => (
                <Link key={category?.name} to={`/products-filters?category=${category?.name}`} className="relative flex h-80 w-56 flex-col mt-4 overflow-hidden rounded-lg p-6 hover:opacity-75 xl:w-auto">
                  <span aria-hidden="true" className="absolute inset-0">
                    <img src={`data:${category?.image.contentType};base64,${category?.image.data.toString("base64")}`} alt={category?.name} className="h-full w-full object-cover object-center"/>
                  </span>
